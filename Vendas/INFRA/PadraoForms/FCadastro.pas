  ...
  begin
    ...
    begin
      ...
      Begin
        if (getSomenteNumeros(TLabeledEdit(Components[i]).Text) = EmptyStr)
         ...
         begin
           MessageDlg(TLabeledEdit(Components[i]).EditLabel.Caption
           + ' é um campo obrigatório, somente válido.', mtInformation, [mbOK], 0);
           TLabeledEdit(Components[i]).SetFocus;
           Result:= True;
           Break;
         end;
    ...
    if (Components[i] is TDBLookupComboBox)
     ...
     begin
        ...
        then
        Begin
           MessageDlg(' Fornecedor é um campo obrigatório.', mtInformation, [mbOK], 0);
...
begin
  //Este Método tem a finalidade de listar informações, das tabelas do Banco de dados.
end;
